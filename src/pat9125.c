...
enum { FLAG_INIT=1<<0, FLAG_UPDATING=1<<2, FLAG_MOTION=1<<7 };
...
command_config_pat9125(uint32_t *args)
{
    ...
    pat->status = FLAG_INIT;
}
DECL_COMMAND(command_config_pat9125,
             "command_config_pat9125 oid=%c i2c_oid=%c");
...
pat9125_get_state(struct pat9125_i2c *pat, uint8_t oid)
{
    ...
    reg = REG_MOTION;
    i2cdev_read(pat->i2c, 1, &reg, 1, &motion);
    pat->status &= ~FLAG_MOTION;
    pat->status |= (motion & FLAG_MOTION);
    if (motion & FLAG_MOTION) {
        // read xyh
        reg = REG_DELTA_X;
    ...
    // TODO: We might need to get the position without
    ...
    sendf("pat9125_state oid=%c epos=%i data=%*s flags=%c",
          oid, position - STEPPER_BIAS, STATE_LEN, data, pat->status);
}
